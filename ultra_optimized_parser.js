...
// Create arrays to collect all data types with broader search
const allNames = [];
const allSessions = [];
const allDepartments = [];
const allPrograms = [];

// Collect all data with improved pattern matching
for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    ...
    // Collect programs with improved pattern
    if ((line.includes('বতমান') && line.includes('াাম') && line.includes(':')) ||
        (line.includes('মাাস (') || line.includes('আডারাজুেয়ট ('))) {
        let programText = '';
        if (line.includes(':')) {
            programText = cleanText(line.split(':')[1] || '');
        } else {
...
for (let v = 0; v < voterNumbers.length; v++) {
    ...
    // Find closest name
    const closestName = findClosestData(voter.lineIndex, allNames, 30);
    if (closestName) {
    ...
    const closestSession = findClosestData(voter.lineIndex, allSessions, 30);
    if (closestSession) {
        voterData.registrationSession = closestSession.text;
    }
    
    // Find closest department (override if found)
    const closestDept = findClosestData(voter.lineIndex, allDepartments, 30);
    if (closestDept) {
        voterData.department = closestDept.text;
    }
    
    // Find closest program
    const closestProgram = findClosestData(voter.lineIndex, allPrograms, 30);
    if (closestProgram) {
        const { program, year } = parseCurrentProgram(closestProgram.text);
        if (program) voterData.currentProgram = program;
        if (year) voterData.year = year;
    }
    
    // Try to infer program/year from surrounding context if not found
    if (!voterData.currentProgram || !voterData.year) {
        for (let i = voter.lineIndex - 20; i <= voter.lineIndex + 20; i++) {
            if (i >= 0 && i < lines.length) {
                const line = lines[i];
                if (line.includes('মাাস') || line.includes('আডারাজুেয়ট')) {
                    const { program, year } = parseCurrentProgram(line);
                    if (program && !voterData.currentProgram) voterData.currentProgram = program;
                    if (year && !voterData.year) voterData.year = year;
                }
            }
        }
    }
